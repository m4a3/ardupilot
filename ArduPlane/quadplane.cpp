...
const AP_Param::GroupInfo QuadPlane::var_info2[] = {
    ...
    AP_SUBGROUPPTR(weathervane, "WVANE_", 26, QuadPlane, AC_WeatherVane),
...
bool QuadPlane::setup(void)
{
    ...
    if (!weathervane) {
        hal.console->printf("%s weathervane\n", strUnableToAllocate);
    }
...
float QuadPlane::get_weathervane_yaw_rate_cds(void)
{
    ...
    */
    ...
    if (weathervane->should_weathervane(plane.channel_rudder->get_control_in(), wp_nav->get_roll(), wp_nav->get_pitch())) {
